@extends('layouts.app', ['page' => trans('sidebar.header.add_product'), 'pageSlug' => 'sales', 'section' => 'transactions'])
                    ...
                    <div class="card-header">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('sales.show', [$sale->id]) }}" class="btn btn-sm btn-primary">{{ trans('button.back') }}</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('sales.product.store', $sale) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('product_id') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-product">{{ trans('inventory.product') }}</label>
                                    <select name="product_id" id="input-product" class="form-select form-control-alternative{{ $errors->has('product_id') ? ' is-invalid' : '' }}" required>
                                        @foreach ($products as $product)
                                            @if($product['id'] == old('product_id'))
                                                <option value="{{$product['id']}}" selected>[{{ $product->category->name }}] {{ $product->name }} - {{ trans('inventory.base_price') }}: {{ $product->price }}$</option>
                                            @else
                                                <option value="{{$product['id']}}">[{{ $product->category->name }}] {{ $product->name }} - {{ trans('inventory.base_price') }}: {{ $product->price }}$</option>
                                            @endif
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group{{ $errors->has('product_id') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-price">{{ trans('inventory.price_unit') }}</label>
                                ...
                                <div class="form-group{{ $errors->has('product_id') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-qty">{{ trans('inventory.quantity') }}</label>
                                ...
                                <div class="form-group{{ $errors->has('product_id') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-total">{{ trans('inventory.total_amount') }}</label>
