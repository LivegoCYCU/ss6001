@extends('layouts.app', ['page' => trans('sidebar.header.register_transfer'), 'pageSlug' => 'transfers', 'section' => 'transactions'])
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-8">
                            <h4 class="card-title">{{ trans('sidebar.header.register_transfer') }}</h4>
                        </div>
                ...
                <div class="card-body">
                    <form method="post" action="{{ route('transfer.store') }}" autocomplete="off">
                        @csrf
                        <h6 class="heading-small text-muted mb-4">{{ trans('transfers.information') }}</h6>
                        <div class="pl-lg-4">
                            <div class="form-group{{ $errors->has('title') ? ' has-danger' : '' }}">
                                <label class="form-control-label" for="input-title">{{ trans('transfers.title') }}</label>
                            ...
                            <div class="form-group{{ $errors->has('sender_method_id') ? ' has-danger' : '' }}">
                                <label class="form-control-label" for="input-method">{{ trans('transfers.sender_method') }}</label>
                            ...
                            <div class="form-group{{ $errors->has('receiver_method_id') ? ' has-danger' : '' }}">
                                <label class="form-control-label" for="input-method">{{ trans('transfers.receiver_method') }}</label>
                            ...
                            <div class="form-group{{ $errors->has('sended_amount') ? ' has-danger' : '' }}">
                                <label class="form-control-label" for="input-sended_amount">{{ trans('transfers.amount_sent') }}</label>
                            ...
                            <div class="form-group{{ $errors->has('received_amount') ? ' has-danger' : '' }}">
                                <label class="form-control-label" for="input-received_amount">{{ trans('transfers.amount_received') }}</label>
                            ...
                            <div class="form-group{{ $errors->has('reference') ? ' has-danger' : '' }}">
                                <label class="form-control-label" for="input-reference">{{ trans('transfers.reference') }}</label>
