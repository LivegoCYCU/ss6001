@extends('layouts.app', ['page' =>  trans('transaction.new'), 'pageSlug' => 'clients', 'section' => 'transactions'])
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <h3 class="mb-0">{{ trans('transaction.new') }}</h3>
                            </div>
                            <div class="col-4 text-right">
                                <a href="{{ route('clients.show', $client) }}" class="btn btn-sm btn-primary">{{ trans('button.back') }}</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('transactions.store') }}" autocomplete="off">
                            ...
                            <h6 class="heading-small text-muted mb-4">{{ trans('transaction.title') }}</h6>
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('payment_method_id') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-method">{{ trans('transaction.payment_method') }}</label>
                                ...
                                <div class="form-group{{ $errors->has('amount') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-amount">{{ trans('transaction.amount') }}</label>
                                ...
                                <div class="form-group{{ $errors->has('reference') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-reference">{{ trans('transaction.reference') }}</label>
