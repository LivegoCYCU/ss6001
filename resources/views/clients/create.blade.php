@extends('layouts.app', ['page' => trans('client.register'), 'pageSlug' => 'clients', 'section' => 'clients'])
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <h3 class="mb-0">{{ trans('client.register') }}</h3>
                            </div>
                            <div class="col-4 text-right">
                                <a href="{{ route('clients.index') }}" class="btn btn-sm btn-primary">{{ trans('button.back') }}</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('clients.store') }}" autocomplete="off">
                            @csrf
                            <h6 class="heading-small text-muted mb-4">{{ trans('client.information') }}</h6>
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-name">{{ trans('client.name') }}</label>
                                ...
                                <div class="row">
                                    <div class="col-1">
                                        ...
                                        <select name="document_type" id="input-document_type" class="form-control form-control-alternative{{ $errors->has('name') ? ' is-invalid' : '' }}" required>
                                            @foreach (['Shopee', 'Advertisement', 'Shop', 'Ubereat', 'foodpanda'] as $document_type)
                                    ...
                                    <div class="col">
                                        <label class="form-control-label" for="input-document_id">{{ trans('client.document_number') }}</label>
                                ...
                                <div class="form-group{{ $errors->has('email') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-email">{{ trans('client.email') }}</label>
                                ...
                                <div class="form-group{{ $errors->has('phone') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-phone">{{ trans('client.telephone') }}</label>
